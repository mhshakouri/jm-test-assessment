...
const injectSSRData = (
  ...
  return template
    ...
    .replace(
      ...
      `<script id="ssr-data">window.__SSR_DATA__ = ${JSON.stringify(ssrData)}</script>`
    )
}
...
app.use('*all', async (req: Request, res: Response) => {
  try {
    ...
    let template: string
    let render: (url: string, cookies?: string | object | null) => Promise<{
      html: string
    ...
    const html = injectSSRData(template, theme, rendered.html, rendered.head, data)
